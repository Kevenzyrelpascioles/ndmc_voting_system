...
if(empty($position)) {
    ...
    $where_clause = "WHERE c.Position = '$position'";
    $filename = str_replace(' ', '_', $position) . "_Candidates";
}
...
foreach ($departments as $dept_id => $dept_name) {
    ...
    if (empty($where_clause)) {
        $full_condition = "WHERE $dept_condition";
    } else {
        $full_condition = "$where_clause AND $dept_condition";
    }
