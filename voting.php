...
if(isset($_POST['save'])){
    ...
    if(empty($governor) || empty($vice) || empty($representative) || 
       $governor == '--Select Candidate--' || $vice == '--Select Candidate--' || $representative == '--Select Candidate--') {
        echo "<script>alert('Please select a candidate for all positions.'); window.location='voting.php';</script>";
        exit();
    }
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="css/enhanced-dashboard.css">
    ...
    <div class="container" style="margin-top: 100px;">
        ...
        echo '<div class="alert alert-info mb-3">
            <div class="row">
                ...
                <div class="col-sm-6"><strong>Year Level:</strong> 1st Year</div>
            </div>
        </div>';
        ?>
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap">
            <h2 class="mb-2 mb-md-0 fw-bold">Please Vote Wisely</h2>
            <a class="btn btn-info" id="help" href="help.php">
        ...
        <form method="post" action="voting.php">
                ...
                <div class="card-body">
                    <div class="candidates-grid row g-3 mb-3">
                        ...
                        while($row=mysqli_fetch_array($governor)){ ?>
                            <div class="col-6 col-md-3 text-center candidate-item">
            ...
            <div class="card shadow-sm mb-4">
                <div class="card-header bg-success text-white" style="background: linear-gradient(90deg, #2e7d32, #4caf50) !important;">
                    <h4 class="m-0">Candidate for 1st Year Representative</h4>
                </div>
                <div class="card-body">
                    <?php 
                    // Get first year representatives once, filtered by course
                    $representative_query = mysqli_query($conn,"SELECT * FROM candidate WHERE Position='1st Year Representative' AND course_id='$voter_course'") or die(mysqli_error($conn));
                    ...
                    <select name="representative" class="form-select mb-3" required>
                        <option value="">--Select Candidate--</option>
                        <?php foreach($representatives as $row){ ?>
                            <option value="<?php echo $row['CandidateID']; ?>"><?php echo $row['FirstName'].' '.$row['LastName']; ?></option>
                        <?php } ?>
                    </select>
                </div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-success" style="background: linear-gradient(135deg, #2e7d32, #4caf50);" data-bs-dismiss="modal">No</button>
