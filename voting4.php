...
if(isset($_POST['save'])){
    ...
    // Validate selections
    if(empty($governor) || empty($vice) || 
       $governor == '--Select Candidate--' || $vice == '--Select Candidate--') {
    ...
    // Ensure at least one representative is selected
    if(($representative1 == '--Select Candidate--' || empty($representative1)) && 
       ($representative2 == '--Select Candidate--' || empty($representative2)) && 
       ($representative3 == '--Select Candidate--' || empty($representative3))) {
    ...
    $_SESSION['temp_representative1'] = $representative1;
    $_SESSION['temp_representative2'] = $representative2;
    $_SESSION['temp_representative3'] = $representative3;
    
    // Redirect to vote confirmation page
    header("Location: vote_confirmation4.php");
    exit();
}
 ?>
    ...
    <div class="container" style="margin-top: 100px;">
        ...
        echo '<div class="alert alert-info mb-3">
            <div class="row">
                ...
                <div class="col-sm-6"><strong>Year Level:</strong> 4th Year</div>
            </div>
        </div>';
        ?>
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap">
            <h2 class="mb-2 mb-md-0 fw-bold">Please Vote Wisely</h2>
            <a class="btn btn-info" id="help" href="help4.php">
                <i class="bi bi-info-circle"></i>
                <span class="d-none d-md-inline">&nbsp;Help</span>
            </a>
        </div>
        <form method="post" action="voting4.php">
            ...
            <div class="card shadow-sm mb-4">
                <div class="card-header bg-success text-white" style="background: linear-gradient(90deg, #2e7d32, #4caf50) !important;">
                    <h4 class="m-0">Candidate for 4th Year Representative</h4>
                </div>
                <div class="card-body">
                    <div class="candidates-grid row g-3 mb-3">
                        <?php 
                        $representative_query=mysqli_query($conn,"SELECT * FROM candidate WHERE Position='4th Year Representative' AND course_id='$voter_course'") or die(mysqli_error($conn));
